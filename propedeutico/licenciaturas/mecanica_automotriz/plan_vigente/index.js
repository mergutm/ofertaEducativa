...
fetch("../../../archivos/carreras/Mecanica_automotriz.json")
            ...
            i == semestre.length - 1 ? 'class="semestre-border"' : ""
                    ...
                    <td>
                        <div class="is-flex is-justify-content-center">
                            <p class="buttons">
                                <a class="button is-small has-tooltip-arrow has-tooltip-multiline" href="../archivos/planes/plan_vigente/${
                                  m.archivo
                        ...
                        <div class="is-flex is-justify-content-center">
                            <p class="buttons">
                                ${
                                  m.nombre == undefined
                                    ? `<div>
                                            <button id="pa_${m.optativa[0].optativa}" class="button is-small has-tooltip-arrow has-tooltip-multiline" onclick="cambioOptativa(1,'op${m.optativa[0].optativa}', '${key}', this); mostrarToast()" data-tooltip="Producción Automotriz">
                                                <span class="icon is-small">
                                                    <i class="has-text-weight-bold"> PA </i>
                                                </span>
                                            </button>
                                            <button id="ve_${m.optativa[0].optativa}" class="button is-small has-tooltip-arrow has-tooltip-multiline" onclick="cambioOptativa(2,'op${m.optativa[0].optativa}', '${key}', this); mostrarToast()" data-tooltip="Vehículos Eléctricos">
                                                <span class="icon is-small">
                                                  <i class="has-text-weight-bold"> VE </i>
                                                </span>
                                            </button>
                                        </div>`
                                    : ""
                                ...
                                <a class="button is-small has-tooltip-arrow has-tooltip-multiline" ${
                                  m.nombre == undefined
                                    ...
                                    : `href="../archivos/planes/plan_vigente/${m.archivo}" target="_blank" download`
                                ...
                                <a class="button is-small has-tooltip-arrow has-tooltip-multiline" ${
                                  m.nombre == undefined
                                    ...
                                    : `href="../archivos/planes/plan_vigente/${m.archivo}" target="_blank"`
...
function cambioOptativa(optativa, id, semestre, e) {
  ...
  const btnPA = document.getElementById(`pa_${idOrignal}`);
  const btnVE = document.getElementById(`ve_${idOrignal}`);

  // establece el nuevo boton activo con la clase is-active
  if (btnPA.id == e.id) {
    btnPA.classList.add("is-active");
    btnPA.classList.add("btn-active");
    btnVE.classList.remove("is-active");
    btnVE.classList.remove("btn-active");
  } else {
    btnVE.classList.add("is-active");
    btnVE.classList.add("btn-active");
    btnPA.classList.remove("is-active");
    btnPA.classList.remove("btn-active");
  }

  const dwnElement = document.getElementById(`dwn_${idOrignal}`);
  const vwElement = document.getElementById(`vw_${idOrignal}`);
  const rutaInicial = "../archivos/planes/plan_vigente/";
