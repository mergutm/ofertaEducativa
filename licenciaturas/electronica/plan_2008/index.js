...
fetch("../../../archivos/carreras/Electronica2008.json")
            ...
            i == semestre.length - 1 ? 'class="semestre-border"' : ""
                    ...
                    <td>
                        <div class="is-flex is-justify-content-center">
                            <p class="buttons">
                                <a class="button is-small has-tooltip-arrow has-tooltip-multiline" href="../archivos/planes/plan_2008/${
                                  m.archivo
                        ...
                        <div class="is-flex is-justify-content-center">
                            <p class="buttons">
                                ${
                                  m.nombre == undefined
                                    ? `<div>
                                            <button id="iot_${m.optativa[0].optativa}" class="button is-small has-tooltip-arrow has-tooltip-multiline" onclick="cambioOptativa(1,'op${m.optativa[0].optativa}', '${key}', this); mostrarToast()" data-tooltip="Control y Automatización">
                                                <span class="icon is-small">
                                                    <i class="has-text-weight-bold"> CyA </i>
                                                </span>
                                            </button>
                                            <button id="ep_${m.optativa[0].optativa}" class="button is-small has-tooltip-arrow has-tooltip-multiline" onclick="cambioOptativa(2,'op${m.optativa[0].optativa}', '${key}', this); mostrarToast()" data-tooltip="Telecomunicaciones">
                                                <span class="icon is-small">
                                                  <i class="has-text-weight-bold"> Tel </i>
                                                </span>
                                            </button>
                                        </div>`
                                    : ""
                                ...
                                <a class="button is-small has-tooltip-arrow has-tooltip-multiline" ${
                                  m.nombre == undefined
                                    ...
                                    : `href="../archivos/planes/plan_2008/${m.archivo}" target="_blank" download`
                                ...
                                <a class="button is-small has-tooltip-arrow has-tooltip-multiline" ${
                                  m.nombre == undefined
                                    ...
                                    : `href="../archivos/planes/plan_2008/${m.archivo}" target="_blank"`
...
function cambioOptativa(optativa, id, semestre, e) {
  ...
  const btnIOT = document.getElementById(`iot_${idOrignal}`);
  const btnEP = document.getElementById(`ep_${idOrignal}`);

  // establece el nuevo boton activo con la clase is-active
  if (btnIOT.id == e.id) {
    btnIOT.classList.add("is-active");
    btnIOT.classList.add("btn-active");
    ...
    btnIOT.classList.remove("is-active");
    btnIOT.classList.remove("btn-active");
  }

  const dwnElement = document.getElementById(`dwn_${idOrignal}`);
  const vwElement = document.getElementById(`vw_${idOrignal}`);
  const rutaInicial = "../archivos/planes/plan_2008/";
